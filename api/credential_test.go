...
type CredentialApiTestSuite struct {
	suite.Suite
...
func (suite *CredentialApiTestSuite) SetupSuite() {
...
func (suite *CredentialApiTestSuite) TestCreate() {
	body, err := json.Marshal(common.Credential{
	...
	req, _ := http.NewRequest(http.MethodPost, suite.server.URL+"/v1/credentials", bytes.NewBuffer(body))
	...
	suite.credentialID = c.ID

	req2, _ := http.NewRequest(http.MethodPost, suite.server.URL+"/v1/credentials", bytes.NewBuffer(body))
...
func (suite *CredentialApiTestSuite) TestDelete() {

	req, _ := http.NewRequest(http.MethodDelete, suite.server.URL+"/v1/credentials/"+suite.credentialID.Hex(), nil)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "application/json")
	req.Header.Set("Authorization", "Bearer "+suite.authHeader.Token)
...
func TestCredentialApiTestSuite(t *testing.T) {
	suite.Run(t, new(CredentialApiTestSuite))
}
